...
describe('normalizeOpenMeteoPrimary — día real con fenómeno (Andorra)', () => {
  ...
  const result = normalizeOpenMeteoPrimary(dailyAndorra)
...
describe('normalizeOpenMeteoPrimary — códigos de solo nubosidad (0-3)', () => {
  it('mapea 0/1/2/3 a despejado/poco_nuboso/nuboso/cubierto', () => {
    expect(normalizeOpenMeteoPrimary(dailyWithCode(0)).sky).toBe('despejado')
    expect(normalizeOpenMeteoPrimary(dailyWithCode(1)).sky).toBe('poco_nuboso')
    expect(normalizeOpenMeteoPrimary(dailyWithCode(2)).sky).toBe('nuboso')
    expect(normalizeOpenMeteoPrimary(dailyWithCode(3)).sky).toBe('cubierto')
  })
})
...
describe('normalizeOpenMeteoPrimary — fenómenos', () => {
  it('tormenta (95, 96, 99): sky null, storm true', () => {
    for (const code of [95, 96, 99]) {
      const result = normalizeOpenMeteoPrimary(dailyWithCode(code))
  ...
  it('nieve (71/73/75/77/85/86): snow.present true derivado de cm, sky null', () => {
    for (const code of [71, 73, 75, 77, 85, 86]) {
      const result = normalizeOpenMeteoPrimary(dailyWithCode(code, { snowfall_sum: [2] }))
  ...
  it('snow.present se deriva de cm incluso si el código no es de nieve (coherencia snow.cm/present)', () => {
    ...
    const result = normalizeOpenMeteoPrimary(dailyWithCode(61, { snowfall_sum: [1.5] }))
  ...
  it('código desconocido: sky/storm/fog quedan null (sin respaldo numérico), no fenómenos ausentes', () => {
    const result = normalizeOpenMeteoPrimary(dailyWithCode(999, { snowfall_sum: [0] }))
    expect(result.sky).toBeNull()
  ...
  it('snow.present sigue derivándose de snowfall_sum aunque el código sea desconocido (sí tiene respaldo numérico)', () => {
    expect(normalizeOpenMeteoPrimary(dailyWithCode(999, { snowfall_sum: [0] })).snow?.present).toBe(false)
    expect(normalizeOpenMeteoPrimary(dailyWithCode(999, { snowfall_sum: [3] })).snow?.present).toBe(true)
  })
})
...
describe('normalizeOpenMeteoComplement — solo campos numéricos (Madrid)', () => {
  ...
  const result = normalizeOpenMeteoComplement(dailyMadrid)
