...
- [ ] En desktop, "POKETIEMPO" queda a la izquierda y "PREVISIÓN · [DÍA DE LA SEMANA] [DÍA] DE [MES]" a la derecha, en mayúsculas, con la fecha de `forecast.date` (sin calcular ningún día en el componente) — el split que ya fija `mission.md`/`tech-stack.md`.
- [ ] "POKETIEMPO" usa la fuente propia Poketiempo Unown (`src/assets/fonts/poketiempo-unown.woff2`); la línea de previsión usa Pixelify Sans (Google Fonts); leyenda, créditos y las temperaturas mínima/máxima del mapa usan Nunito Sans (Google Fonts). Ninguna de las dos fuentes de Google Fonts se añade como dependencia JS — solo `<link>` en `index.html`.
- [ ] El color de relleno de "POKETIEMPO" y de la línea de previsión cambia igual entre los dos, según la categoría térmica que predomina entre los 74 `temperature.maxC` del forecast: gelid (`< 0`), cold (`0–<10`), neutral (`10–<26`), heat (`26–<35`), sweltering (`>= 35`). Empate en el recuento se resuelve con la categoría en la que cae la mediana de las 74 máximas — determinista, no depende del orden del array.
- [ ] "POKETIEMPO" lleva borde negro fijo en las cinco categorías (`$color-near-black`) con `font-weight: 700` (negrita sintética), stroke CSS real (`-webkit-text-stroke`), nunca una pila de `text-shadow`. La línea de previsión, mucho más pequeña, lleva su propio borde de mood (`--mood-border`, un matiz del relleno, no negro) y su propio grosor, más fino que el del título.
- [ ] Sin responsive avanzado todavía (se decide en la feature final cómo se apila/reorganiza en móvil): esta feature solo garantiza que el split no produce overflow horizontal en los anchos que ya cubre la 004.

**Mapa — paleta y territorios**

- [ ] El mar (`#B9FFFD`) rellena el fondo completo del mapa principal y del recuadro de Canarias.
- [ ] España, Baleares, Ceuta, Melilla y Canarias comparten el mismo color (`#FFEBF6`); Portugal (`#F6FFEC`) y Andorra (`#FFF3B0`) llevan el suyo propio; el contexto de Marruecos/Argelia reutiliza el color de España, visiblemente más tenue (opacidad reducida); el marco de Canarias usa `#42C7EC`.
- [ ] España, Portugal, Andorra, Baleares, Ceuta y Melilla son paths SVG independientes (generados en `scripts/build-map.ts` con la misma proyección de la 004) — no una única silueta combinada resuelta con hacks de CSS.
- [ ] `locations.ts` y las coordenadas de los 74 lugares no cambian.

**Mapa — temperaturas por marcador**

- [ ] Cada `LocationMarker` con forecast muestra su mínima y máxima, en ese orden, superpuestas en la zona inferior del sprite, centradas horizontalmente, con el símbolo `°`, redondeadas al entero (el redondeo es solo de presentación).
- [ ] Cada número se colorea de forma independiente según su propio valor, en sus seis franjas (`< 0`, `0–<10`, `10–20`, `21–25`, `26–34`, `>= 35`), con `paint-order: stroke fill` cuando lleva borde.
- [ ] El nombre accesible del marcador incluye las temperaturas cuando están disponibles (ej. "Granada, mínima 12 grados, máxima 24 grados"); un marcador sin forecast sigue tolerado (solo nombre, sin temperatura ni sprite).
- [ ] El mapa sigue mostrando exactamente 74 `LocationMarker`.

**Leyenda**

- [ ] Muestra el encabezado visible "Leyenda" (fuente Poketiempo Unown), que además da el nombre accesible a la sección vía `aria-labelledby` (sin `aria-label` redundante).
- [ ] Muestra únicamente los `PokedexId` que `pickMapPokemon(assignPokemon(...))` produce para al menos uno de los 74 lugares ese día — nunca los 24 fijos.
- [ ] Sin duplicados: un Pokémon que aparece en varios lugares sale una sola vez.
- [ ] Orden coherente con `MAP_PRIORITY` (exportada desde `pick-map-pokemon.ts`, sin copiarla).
- [ ] Cada entrada muestra sprite (mismo `spriteSources` del mapa) + la descripción real de la condición que le corresponde (metadata fija, no inferida por sprite — `castform-sun` nunca dice "soleado": sigue asignándose por temperatura sola además de por cielo despejado, así que puede tocarle a un día nublado a esa temperatura).
- [ ] HTML, no SVG.

**Créditos**

- [ ] "Datos meteorológicos: AEMET · IPMA · Open-Meteo" y "PokéTiempo original: Gabriel Ortega Díaz", en HTML semántico, sin URLs inventadas, fuera del SVG del mapa.

**Generales**

- [ ] Los 25 `PokedexId` tienen metadata de leyenda.
- [ ] `assignPokemon()` y `MAP_PRIORITY` solo cambian en lo descrito en "Piezas del motor de asignación que necesita la leyenda" (arriba) — ningún otro eje, franja ni prioridad se toca.
- [ ] Ningún significado se comunica solo por color (el texto de la leyenda y el nombre accesible de cada marcador siguen siendo la fuente de verdad, el color es refuerzo).
- [ ] `sky === 'despejado'` asigna `castform-sun` (además de la franja 15–25°C, que sigue igual); `assignPokemon` no devuelve el mismo `PokedexId` dos veces cuando dos ejes coinciden.
- [ ] `waveHeightM >= 2,5` asigna Mega Gyarados aunque no haya aviso rojo costero activo; con aviso, sigue disparando igual que antes.
- [ ] Un día despejado con temperatura fuera de 15–25°C sigue mostrando en el mapa el Pokémon de su temperatura real, no `castform-sun` (verifica el ajuste de `MAP_PRIORITY`).
- [ ] `wind.speedKmh >= 40` y `temperature.maxC >= 30` a la vez asignan Moltres; si falta cualquiera de los dos, no.
