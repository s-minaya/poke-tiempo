...
4. deja que la IA redacte esas afirmaciones ya cerradas,
5. usa un fallback local determinista si la IA falla.

La IA **nunca** puede: reasignar Pokémon, inventar temperaturas, inventar lluvia/viento/avisos, añadir lugares que no estén en el hecho que redacta, ni afirmar fenómenos que no existan en el `DayPlan`. No recibe el `forecast.json`, ni los `NarrativeFact`: recibe **claims**, afirmaciones en español ya resueltas por el dominio, y su único trabajo es la realización lingüística. Lo que devuelve pasa por una guarda determinista que rechaza cualquier cifra, lugar, Pokémon o nivel de aviso que no estuviera en los claims de ese diálogo.
...
- **006 cerrada** — el botón `EMPEZAR` existe y el flujo de entrada de `App.tsx` tiene un punto natural donde montar a Oak, entre la portada y el mapa.
...
- La escena de Oak (`src/components/ProfessorOak/`): retrato, bocadillo clásico, máquina de escribir y avance, entre la portada y el mapa.
...
- [x] Oak no contiene ninguna regla meteorológica ni de asignación Pokémon propia.
- [x] **Oak nunca nombra un Pokémon que no se vea en el mapa.** Todo `PokedexId` de un hecho procede de `pickMapPokemon`; ningún candidato descartado por `assignPokemon` llega al relato.
- [x] No existe ninguna tabla nueva de "qué Pokémon es importante". La importancia se deriva de `MAP_PRIORITY` y de recuentos reales del día.
- [x] "Condición significativa" se decide con la frontera que `MAP_PRIORITY` ya documenta (por delante de `hoppip`), no con una lista nueva.
- [x] Los 3 diálogos se generan siempre, aunque falle la IA: fallback local determinista, sin red.
- [x] Ningún dato meteorológico (temperatura, mm, km/h, avisos, niveles) puede ser inventado por la IA — solo redacta claims ya resueltos, y una guarda determinista rechaza cualquier entidad factual que no estuviera en ellos.
- [x] Ningún hecho se repite en más de un diálogo del mismo día.
- [x] Los diálogos hablan de **mañana**: `forecast.date` es `targetDate` (`target-date.ts`), no hoy.
- [x] El modo `alerta` solo se activa desde una señal explícita de aviso oficial activo en `forecast.date`, nunca inferida del Pokémon asignado, y tiene prioridad absoluta sobre los demás modos.
...
- [x] El pipeline nunca sobrescribe un `oak-today.json` válido con uno inválido.
- [x] El historial guarda solo IDs y categorías, nunca el texto generado.
- [x] **El historial es idempotente por fecha.** Varias generaciones para el mismo `forecast.date` no cuentan como varios días: al leer se ignora la entrada de la fecha objetivo, al escribir se hace upsert, y nunca hay dos entradas con la misma `date`.
- [x] El artifact del `forecast.json` se sigue subiendo inmediatamente después del fetch: un fallo de Oak nunca puede impedir conservar un forecast válido.
- [x] Sin dependencias nuevas de runtime. Sin SDK de IA si `fetch` de Node resuelve el contrato.
