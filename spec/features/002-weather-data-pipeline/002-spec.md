...
- [x] `alerts` distingue "consultado sin avisos", "fuente sin este producto" y "error en la consulta de hoy" — nunca los tres casos colapsados en un mismo `null`. `marine` distingue igual "no aplica (interior)" de "costero pero falló hoy" — nunca ambos como el mismo `null`. Ambos son independientes del `weather`: que fallen no invalida el lugar.
